...
func main() {
	mq := NewRabbitMQ("queue_publisher", "exchange_publisher", "key1")
	defer mq.ReleaseRes()
	...
	*/
	...
	err = mq.Channel.ExchangeDeclare(
		mq.Exchange, //交换器名
		"topic",     //exchange type：一般用direct、fanout、topic
		true,        // 是否持久化
	...
	err = mq.Channel.QueueBind(
		"project.apple", // 绑定的队列名称
		"project.apple", // bindkey 用于消息路由分发的key
	...
	err = mq.Channel.QueueBind(
		"project.peer", // 绑定的队列名称
		"project.#",    // bindkey 用于消息路由分发的key
		mq.Exchange,    // 绑定的exchange名
		false,          // 是否阻塞
		nil,            // 额外属性
	)
	...
	// 4.发送消息
	for {
		var keys = []string{"project.peer", "project.apple"}
		var values = []string{"hello.peer", "hello.apple"}
		idx := rand.Intn(2)
		key := keys[idx]
		value := values[idx]
